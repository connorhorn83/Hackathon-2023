...
from data.HealthEffects import get_he_data
# from BarcodeAPIDataSplit import run_function
# from Score_Calculator import calc_scores
# from HealthEffects import get_he_data
import wikipedia
...
def search_components(components_list):
    ...
    # get health effects data
    he_data = get_he_data()
    he_data = he_data.set_index('Name', drop=False)
    
    df_combined['Health Effects'] = 'No health effects found'
    df_combined['Name'] = df_combined['Name'].str.lower().str.strip()
    
    # add health effects to df_combined
    for component in df_combined['Name']:
        component = component.lower().strip()
        i = 0
        for i in range(len(he_data['Name'].values)):
            if component == he_data['Name'].values[i]:
                df_combined.loc[df_combined['Name'] == component, 'Health Effects'] =  he_data.loc[he_data['Name'] == component, 'Health Effects'].values[i]


    # reorder columns
    column_reorder = ['Name', 'Use', 'Health Effects', 'Summary']
    df_combined = df_combined.reindex(columns=column_reorder)
    return df_combined
